...
class RfbProto {
  ...
  void readServerInit() throws IOException {
   ...
   // framebufferWidth  = AppConnection.getInstance().AppWidth();
    //framebufferHeight = AppConnection.getInstance().AppHeight();
  ...
  */
  ...
  {
	  	int x_forbidden_win = 1260;
	  	int y_forbidden_win = 26;
	  	int x_forbidden_linux = 1024 - 26;
	  	int y_forbidden_linux = 26;
	    eventBufLen = 0;
	    writeModifierKeyEvents(modifiers);
	    //dhm 9.24
	    if (VncCanvasActivity.isLinux)
	    	x += VncCanvasActivity.deltaX;
	    if (!VncCanvasActivity.isLinux)
	    {
	    	if (y < y_forbidden_win && x < x_forbidden_win) return ;
	    }
	    else
	    {
	    	if (y < y_forbidden_linux && x < x_forbidden_linux) return ;
	    }
